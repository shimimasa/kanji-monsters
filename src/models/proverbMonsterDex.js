// src/models/proverbMonsterDex.js
// 中学生用ことわざモンスターデックス管理モジュール
// localStorage に Set<string> 形式で永続化
// 使用キー: 'krb_proverb_monster_dex', 'krb_seen_proverb_monsters'

import DataSync from '../services/firebase/dataSync.js';

const STORAGE_KEY = 'krb_proverb_monster_dex';
const SEEN_STORAGE_KEY = 'krb_seen_proverb_monsters';
...
export function loadDex() {
  try {
    ...
    console.error('proverbMonsterDex: 図鑑のロードに失敗しました', e);
    return new Set();
  }
}
...
export function loadSeenMonsters() {
  try {
    ...
    console.error('proverbMonsterDex: 確認済みモンスターのロードに失敗しました', e);
    return new Set();
  }
}
...
export function saveDex(dex) {
  try {
    ...
    console.error('proverbMonsterDex: 図鑑の保存に失敗しました', e);
  }
}
...
export function saveSeenMonsters(seenSet) {
  try {
    ...
    console.error('proverbMonsterDex: 確認済みモンスターの保存に失敗しました', e);
  }
}
