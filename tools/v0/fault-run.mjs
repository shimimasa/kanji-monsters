import {diagnostic} from './fault-diagnostic.mjs';
...
data.toolHashes={};for(const file of ['fault-run.mjs','fault-probe.js','fault-diagnostic.mjs','probe.js','source-points.mjs','chrome.mjs','foreground-monitor.mjs','q-capture.mjs']){const body=await readFile(new URL(file,import.meta.url));data.toolHashes[file]=hash(body);await writeFile(prefix+'-source-'+file,body,{flag:'wx'});}
...
try{
 await c.send('Page.addScriptToEvaluateOnNewDocument',{source:'globalThis.__v0FaultKind='+JSON.stringify(process.argv[5])+';'+await readFile(new URL('./fault-probe.js',import.meta.url),'utf8')});
 if(process.argv[5]==='import-failure')await c.send('Network.setBlockedURLs',{urls:['*babylonPrimitiveRenderer-*']});
