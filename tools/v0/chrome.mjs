...
export async function launchChrome(executable, profile, {headless=false}={}) {
  ...
  const child = spawn(executable, [`--user-data-dir=${profile}`,'--remote-debugging-port=0',
    ...(headless?['--headless=new']:[]),'--no-first-run','--no-default-browser-check','--disable-sync','--disable-background-networking',
  ...
  const targets=await call('Target.getTargets');const pages=targets.targetInfos.filter(t=>t.type==='page'&&t.url==='about:blank');if(pages.length!==1)throw Error('Expected one new dedicated blank page');const {targetId}=pages[0];await call('Target.activateTarget',{targetId});
  ...
  return {child,targetId,call,send,evaluate,on:cb=>listeners.push(cb),
