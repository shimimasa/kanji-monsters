...
import {launchChrome,sleep} from '../v0/chrome.mjs';
import {points,names,bundle} from '../v0/source-points.mjs';
import {qCapture} from '../v0/q-capture.mjs';
import {monitorForeground} from '../v0/foreground-monitor.mjs';
import {fileURLToPath} from 'node:url';
import {canonical,hash} from '../experiment/common.mjs';
const [role='baseline',attempt='functional-01',mode='diagnostic']=process.argv.slice(2);
const root='C:/kanji-game-latest/artifacts/yomitabi-e0/worktrees/experiment/artifacts/v1a/integration-01', origin=`http://127.0.0.1:${role==='baseline'?49721:49722}`;
const prefix=path.join(root,role+'-'+attempt), profile=prefix+'-profile';
const data={role,attempt,mode,origin,profile,protocol:'V1a-2 headed functional; NO performance comparison',startedAt:new Date().toISOString(),requests:[],responses:[],finished:[],failed:[],steps:[],logs:[]};
data.toolHashes={};for(const file of ['run.mjs','diagnostic.mjs','diagnostic-probe.js']){const body=await readFile(new URL(file,import.meta.url));data.toolHashes[file]=hash(body);await writeFile(prefix+'-source-'+file,body,{flag:'wx'});}
const c=await launchChrome('C:/Program Files/Google/Chrome/Application/chrome.exe',profile,{headless:false});let fatal,monitor,qImages;const imageWrites=[];data.casts=[];
...
async function action(kind){
 ...
 await c.screenshot(prefix+'-action-'+kind+'.png');
 if(kind==='wrong' && before.enemy.id==='HKD-E01' && await ev("document.querySelector('#gameCanvas').classList.contains('yomitabi-3d-active')")){
  await until("__b0.resources().visual?.clip==='attack'",'GLB enemy attack');await snap('GLB-attack');await c.screenshot(prefix+'-GLB-attack.png');
 }
...
const observe=`(()=>{const o=globalThis.__b0obs={frames:[],events:[],longTasks:[],label:'navigation',pending:null};let lastTitle=false,lastReady=false,lastStyle='',lastEnemyAction='';
 ...
 function frame(t){if(false)o.frames.push([t,o.label]);const title=!!(globalThis.fsm?.currentState===globalThis.fsm?.states?.title&&globalThis.fsm?.currentState?.playButton);if(title&&!lastTitle)o.events.push({kind:'T_visible_proxy',t});lastTitle=title;requestAnimationFrame(frame);}requestAnimationFrame(frame);
...
try{
 ...
 monitor={finish:async()=>{},records:[]}; // Functional diagnostics only; not a foreground performance run.
data.chromePid=c.child.pid;data.targetId=c.targetId;
 ...
 assert.equal(mode,'diagnostic','This tool is functional-only');
 ...
 await c.send('Page.addScriptToEvaluateOnNewDocument',{source:'globalThis.__b05Boundary='+JSON.stringify(mode==='boundary')+';'+await readFile(new URL('../v0/probe.js',import.meta.url),'utf8')+observe});
 ...
 await until("document.querySelector('#gameCanvas').classList.contains('yomitabi-3d-active')",'GLB ready');await sleep(400);
 if(mode!=='control'&&process.env.V1A_SAMPLE_TEST!=='1'){
 ...
 data.functionalLongTasks=await ev('__b0obs.longTasks');
 ...
 if(mode==='diagnostic'&&process.env.V1A_ACTIONS_ONLY!=='1')await (process.env.V1A_FAULTS==='1'?faults:diagnostic)({c,ev,until,stage,leave,guide,snap,action,canvas,toggle,data,prefix});
 data.finalValidation=await ev("(async()=>{const a=await import('/__experiment/save-validator.js');return a.readSaveState().status})()");assert.equal(data.finalValidation,'valid');
