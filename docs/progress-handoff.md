...
直近の作業: 監査①（`docs/audit-prompts.md` の①）で見つかった隠れバグ11件の修正 ＋ 伝説モンスター10体の追加
...
**監査②（ポジティブフレーミング監査）を Fable に実行させる。**

- プロンプト全文: `docs/audit-prompts.md` の「② ポジティブフレーミング監査（体験の質）」を**そのまま**渡す
- 実行方法: サブエージェント（Fable）に投げる。①と同じく **報告のみ・コード修正はさせない**
- ①のときの所見（`docs/audit-prompts.md` 末尾）を踏まえ、②のプロンプトには次の一文を足すこと:
  「**見つけた問題ごとに、同じ原因が他に何件あるかを数えて報告する**」
  （①ではこれが無くて、影響範囲の見積もりを外した）
- ②の結果は、こちらで裏取りしてから修正に入る（①では10件中6件を裏取りし、すべて事実だった）
- **②が先に扱うべき既知の1件**: 力だめし（`gradeQuizScreen.js:123-124`）が「不正解…」を赤字 `#e74c3c` で表示している。
  Sprint 1 で決めた「赤字誤答→中立色」「しっぱい系の語を使わない」方針と正面から矛盾する。
  文言の最終決定はユーザーの領分なので、代替案を出して確認を取る
