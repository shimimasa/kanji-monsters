...
- **③の直後に効いてくる注意**: 力だめしの結果画面から「復習に挑戦」を押しても、
  初回予定が翌朝になったため**その場では出題対象が無く、無言でステージ選択へ戻る**。
  この空振りの案内は未実装（A-6 参照）

#### 監査③の結果（2026-08-28 実施・完了）

10件の指摘のうち **8件を実コードとデータで裏取りし、すべて事実**だった。承認のうえ5件を修正:
...
6. **監査③の積み残し（下記5件）の判断**。全件リストは `docs/audit3-findings/`。
   - **力だめし→「復習に挑戦」の空振り**（最優先）: 初回予定を翌朝にしたため、直後は出題対象が無く
     `reviewStage.enter` が無言で `stageSelect` へ戻す（`reviewStage.js:33-35`）。**押しても何も起きないように見える**。
     案: 「きょうの ぶんは ぜんぶ おわったよ！ また あした ここで まってるね」を出してから戻す。文言は要判断
   ...
   - 参考: 監査③は #7「伸びない週の表示」も指摘（勝利画面の「はじめて読めた漢字: **0個**」は周回すると常態化、
     プロフィールの「先週のペースまで あと**N回**」は N が60〜100になり得る）。**文言の判断待ちで未着手**
