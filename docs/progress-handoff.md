...
- プロンプト全文: `docs/audit-prompts.md` の「② 体験の質の監査（ポジティブフレーミング）」を**そのまま**渡す（2026-08-28 に①の結果を織り込んで改訂済み）
- 実行方法: サブエージェント（Fable）に投げる。①と同じく **報告のみ・コード修正はさせない**
- **プロンプトに追記は不要**。①の所見（全件数える／内部文言を数えない／到達不能画面を除く 等）は
  すでに②の本文に反映済み。変更点は②の直後の折りたたみに書いてある
