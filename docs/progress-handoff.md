...
**監査①②③はすべて完了した。次はスモークテストと、下の A-6 の判断。**

- 監査③で学習設計に手が入った（チェックポイント・ボーナス解禁ゲート・SRSパラメータ）。
  **`docs/smoke-test-checklist.md` に沿って、少なくとも次の4点を実機で確認すること**:
  1. ステージで5体倒す → わざと力尽きる → 「もういちど！」で**6体目から**始まるか
  2. クリア済みステージに再入場すると**1体目から**始まるか（旗が外れているか）
  3. バトルで読みちがえた字が、**その日のうちには**「今日の復習」に出ず、翌日出るか
  4. 「今日の復習」を1周したあと、プロフィールの「こんしゅうのがんばり」が増えるか
- 5. 「今日の復習」を出題対象が無い状態で開くと、ひと言出してから戻るか
  （力だめしの直後に「復習に挑戦」を押すと、初回予定が翌朝なのでこの状態になる）
